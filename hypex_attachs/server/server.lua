...
vCLIENT = Tunnel.getInterface("hypex_attachs")
src = {}
Tunnel.bindInterface("hypex_attachs", src)
Proxy.addInterface("hypex_attachs", src)
...
Citizen.CreateThread(
    function()
        zof.prepare(
            ...
            [[            CREATE TABLE IF NOT EXISTS hypex_attachs(                user_id INTEGER,                attachs text,                PRIMARY KEY (`user_id`) USING BTREE            )        ]]
        )
        zof.prepare("zo/check_exists_table_attachs", "SHOW TABLES LIKE 'hypex_attachs'")
        zof.prepare("zo/insert_attachs", "INSERT INTO hypex_attachs(user_id, attachs) VALUES(@user_id, @attachs)")
        zof.prepare("zo/update_attachs", "UPDATE hypex_attachs SET attachs = @attachs WHERE user_id = @user_id")
        zof.prepare("zo/get_attachs", "SELECT * FROM hypex_attachs WHERE user_id = @user_id")
        async(
            function()
